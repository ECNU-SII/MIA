...
class SearchTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        try:
            ...
            # --- START: 新增的摘要生成逻辑 (V3 - 兼容字符串和列表) ---
            
            final_text_parts = []
            final_text = result_text  # 默认返回原始 JSON
            ...
            try:
                ...
                if isinstance(parsed_result, dict) and "result" in parsed_result:
                    results_data = parsed_result["result"]

                    # 2.A 如果 "result" 是一个大字符串 (你指出的情况)
                    if isinstance(results_data, str):
                        # 3.A 用 "---" 分割字符串
                        split_results = results_data.split("---")
                        
                        if len(split_results) == len(query_list_from_params):
                            for query, result_chunk_str in zip(query_list_from_params, split_results):
                                # 清理掉可能存在的额外换行符
                                result_chunk_str = result_chunk_str.strip()
                                
                                # (一个简单的估计算法) 通过 "Doc " 出现的次数来计算结果数
                                count = result_chunk_str.count("Doc ") 
                                
                                # 5. 生成摘要行
                                summary_line = f"A search for '{query}' found {count} results."
                                
                                # 6. 为该查询的结果创建单独的 JSON 对象
                                partial_json_obj = {"result": result_chunk_str}
                                json_line = json.dumps(partial_json_obj, ensure_ascii=False)
                                
                                # 7. 将摘要和对应的 JSON 合并为一行
                                final_text_parts.append(f"{summary_line} {json_line}")

                    # 2.B 如果 "result" 是一个列表 (我之前的假设)
                    elif isinstance(results_data, list):
                        # 3.B 确保查询列表和结果列表长度一致
                        if len(results_data) == len(query_list_from_params):
                            for query, query_results_list in zip(query_list_from_params, results_data):
                                count = len(query_results_list) if isinstance(query_results_list, list) else 0
                                summary_line = f"A search for '{query}' found {count} results."
                                
                                # (我们保留 [query_results_list] 这种嵌套列表结构)
                                partial_json_obj = {"result": [query_results_list]} 
                                json_line = json.dumps(partial_json_obj, ensure_ascii=False)
                                final_text_parts.append(f"{summary_line} {json_line}")

                    # 8. 如果成功生成了部分，则用换行符连接它们
                    if final_text_parts:
                        final_text = "\n".join(final_text_parts)
                
                # else: 如果 JSON 结构不是预期的 { "result": ... }，
                # (例如返回的是 {"error": ...})，则 final_text 
                # 将保持为原始的 result_text，这是安全的。

            except json.JSONDecodeError:
                logger.warning(f"[SearchTool] Result was not valid JSON: {result_text}")
                pass # final_text 已经是 result_text 了

            # --- END: 新增的摘要生成逻辑 (V3 - 兼容字符串和列表) ---
            final_text = final_text.replace("<image>", "")
            return ToolResponse(text=final_text), 0.0, metrics # <-- 返回修改后的 final_text

        except Exception as e:
            error_result = json.dumps({"result": f"Search execution failed: {e}"})
            logger.error(f"[SearchTool] Execution failed: {e}")
            error_result = error_result.replace("<image>", "")
