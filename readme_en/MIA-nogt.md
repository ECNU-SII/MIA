...
Configure and deploy the `Planner`:
```bash
bash deploy.sh
```

Configure the run script `run-nogt.sh`:
- `MEMORY_URL`: `Memory Manager` service, set to the `Qwen3-32B` service URL.
- `PLAN_URL`: `Planner` service, set to the deployed `Planner` service URL.
...
Navigate to `/Inference/MIA-noTTRL-nogt`.
