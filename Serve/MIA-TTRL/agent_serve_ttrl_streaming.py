...
@app.route("/streaming/register_images", methods=["POST"])
def streaming_register_images():
    """流式注册：与 ``load_parquet_images`` 单条逻辑一致，存 ``[images[0]]``，并初始化 ``memory_bank``。"""
    ...
    if entries is None:
        if data.get("data_id") is not None:
            ...
            return jsonify({"error": "Missing 'entries' or top-level 'data_id'"}), 400
    ...
    for ent in entries:
        ...
        images = ent.get("images", [])
        if not isinstance(images, (list, tuple)):
            images = [images] if images is not None else []

        if len(images) > 0:
            DATA_ID_TO_IMAGES[data_id] = [images[0]]
        else:
            DATA_ID_TO_IMAGES[data_id] = []

        if data_id not in memory_bank:
            memory_bank[data_id] = []

        registered += 1

    print(f"Registered {registered} images.")
    return jsonify(
        {
            "status": "ok",
            "registered": registered,
            "total_data_ids": len(DATA_ID_TO_IMAGES),
        }
    )


@app.route("/streaming/health", methods=["GET"])
def streaming_health():
    return jsonify(
        {
            "status": "ok",
            "registered_data_ids": len(DATA_ID_TO_IMAGES),
            "memory_bank_keys": len(memory_bank),
        }
    )
...
def consolidate_memories():
    try:
        ...
        memory_bank[data_id] = []
        print(f"consolidate_memories error: {e!r}")
        traceback.print_exc()
        return jsonify({
            "status": "error",
            "message": str(e),
        ...
        for data_id in tqdm(data_ids_to_process, desc="Evaluating Plans"):
            ...
            for idx, entry in enumerate(entries):
                ...
                if plan_content:
                    ...
                    if "I have a revised plan" in entry["trace"]:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Memory-powered Question Answering Server (streaming, no parquet by default)')
