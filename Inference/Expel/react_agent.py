...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def _run(self, data: Union[str, Dict], model: str, **kwargs) -> Dict:
        ...
        try:
            ...
            plan_prompt = f"\nHere is a rule for your reference:\n{plan}\nBegin your answer:\n"
        except:
            plan = ""
            planner_messages = []
            pos_indices = []
        ...
        while num_llm_calls_available > 0:
            ...
            if tool_call is not None and tool_call.strip():
                ...
                try:
                    ...
                    response = requests.post(replan_judge_url, json=[{"question": question, "traj": trace, "result": prediction}])
                    results = response.json()
                    judge_replan = results[0]
                    if not judge_replan and reflection_times < max_reflection_times:
                        data = {
                            "question": question, 
                            "image_caption": image_caption, 
                            "traj": trace,
