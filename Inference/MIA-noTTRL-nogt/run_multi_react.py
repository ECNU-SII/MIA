...
from judge_nogt import evaluate_agent_without_gt
...
if __name__ == "__main__":
    ...
    memory_base_url = os.environ.get('MEMORY_BASE_URL')
    batch_memory_url = f"{memory_base_url}/batch_memory_save"
    save_memory_url = f"{memory_base_url}/save_memory"
    clear_memory_url = f"{memory_base_url}/clear_memory"
             
             
    save_memory_path = os.path.join(dataset_dir, f"memory.jsonl")
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for future in tqdm(as_completed(future_to_task), total=len(tasks_to_run_all), desc="Processing All Rollouts"):
                ...
                try:
                    ...
                    used_memory_indices = result["used_memory_indices"]
                    judgement_data = evaluate_agent_without_gt(question = result["question"], trajectory = result["trace"], final_output = result["answer"])
                    judgement_nogt = judgement_data["is_correct"]
                    judgement = judge_answer(result["question"], result["prediction"], result["answer"])
                    evaluator_1_feedback = judgement_data["evaluator_1_feedback"]
                    evaluator_2_feedback = judgement_data["evaluator_2_feedback"]
                    evaluator_3_feedback = judgement_data["evaluator_3_feedback"]
                    if "image_caption" in item.keys():
                    ...
                    image_caption = result['image_caption'][0] if len(result['image_caption']) > 0 else ""
                    data = [{
                        "data_id": result["id"],
                    ...
                    try:
                        ...
                        response = requests.post(batch_memory_url, json=data)
                        reslut = response.json()
                        payload = {
                            "save_path": save_memory_path
                        }
                        response = requests.post(save_memory_url, json=payload)
                    except:
                        pass
