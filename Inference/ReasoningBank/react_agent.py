...
reasoning_bank = ReasoningBank(config_path="./config.yaml")
...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def _run(self, data: Union[str, Dict], model: str, **kwargs) -> Dict:
        ...
        try:
            retrieved_memories = reasoning_bank.retrieve_memories(question, k=1)
            
            retrieved_memories_str = ""
            for i in range(len(retrieved_memories)):
                memory = retrieved_memories[i]
                # print(memory)
                memory_data = json.loads(memory["distilled_items"])
                t = str(i +1)
                retrieved_memories_str += f"Example {t}:\n"
                retrieved_memories_str += f"Title: {memory_data['title']}\n"
                retrieved_memories_str += f"Description: {memory_data['description']}\n"
                retrieved_memories_str += f"Content: {memory_data['content']}\n"
            
            if retrieved_memories_str:
