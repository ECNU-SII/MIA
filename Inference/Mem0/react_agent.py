...
# if os.path.exists("./my_mem0_db_clean"):
#     shutil.rmtree("./my_mem0_db_clean")

VLLM_API_BASE = "VLLM_API_BASE/8002/v1"
VLLM_API_KEY = "EMPTY"

# 本地嵌入模型路径
LOCAL_EMBED_MODEL_PATH = "/your_path/bert/sup-simcse-bert-base-uncased"
# LOCAL_EMBED_MODEL_PATH = "./"
config = {
    "llm": {
        "provider": "openai",  # 利用 vLLM 的 OpenAI 兼容接口
        "config": {
            "api_key": VLLM_API_KEY,
            "model": "qwen",  # 或你加载的具体模型名，需与 vLLM 启动时一致
            "temperature": 0.1,
            "max_tokens": 4096,
            "openai_base_url": VLLM_API_BASE,  # 关键：指向本地 vLLM
        }
    },
    "embedder": {
        "provider": "huggingface",  # 使用本地 HF 模型
        "config": {
            "model": LOCAL_EMBED_MODEL_PATH,
        }
    },
    "vector_store": {
        "provider": "chroma",
        "config": {
            "path": "./my_mem0_db_clean",  # 明确指定路径
        }
    },
...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def _run(self, data: Union[str, Dict], model: str, **kwargs) -> Dict:
        ...
        try: 
            relevant_memories = memory_client.search(query=question, user_id="deepresearch", limit=3)
            memories_str = "\n".join(f"- {entry['memory']}" for entry in relevant_memories["results"])
            memories_str = memories_str.strip()
            print()
            print()
            print("FIND YES", memories_str)
            print()
            print()
            if memories_str:
                mem_prompt = f"\nHere are some memories for your reference:\n{memories_str}\n"
            else:
                mem_prompt = ""
        except Exception as e:
            print()
            print()
            print("USE Error retrieving memories:", str(e))
            print()
            print()
            mem_prompt = ""
        ...
        if images:
            question_1 = SYSTEM_PROMPT_IMAGE + PROMPT_IMAGE.format(mem_prompt=mem_prompt) + question 
        else:
            question_1 = SYSTEM_PROMPT_TEXT + PROMPT_TEXT.format(mem_prompt=mem_prompt) + question
        ...
        while num_llm_calls_available > 0:
            ...
            if tool_call is not None and tool_call.strip():
                ...
                try:
                    # memory_client.add(messages[-2:], metadata={ "app_id" : app_id}, user_id="deepresearch")
                    memory_client.add(messages, metadata={ "app_id" : app_id}, user_id="deepresearch")
                    print("ADD YES")
                except Exception as e:
                    print("ADD Error retrieving memories:", str(e))
                break
