...
MEMORY_MODE = os.getenv("MEMORY_MODE", "default")
...
if __name__ == "__main__":
    ...
    if MEMORY_MODE == "amem":
        batch_memory_url = f"{memory_base_url}/add_memory"    # a-mem
        clear_memory_url = f"{memory_base_url}/clear_memory"       # a-mem
    else:
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for future in tqdm(as_completed(future_to_task), total=len(tasks_to_run_all), desc="Processing All Rollouts"):
                ...
                try:
                    ...
                    try:
                        if MEMORY_MODE == "amem":
                            response = requests.post(batch_memory_url, json=data[0])  # a-mem
                        else:
