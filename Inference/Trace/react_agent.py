...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def _run(self, data: Union[str, Dict], model: str, **kwargs) -> Dict:
        ...
        memory_url = f"{memory_base_url}/memory"
        # memory_url = f"{memory_base_url}/memory_context"    # a-mem
        ...
        response = requests.post(memory_url, json=[{"data_id": ids, "question": question, "image_caption": image_caption}])
        memorys = response.json()
        context = memorys[0]['context']
        
        # response = requests.post(memory_url, json={"question": question})   # a-mem
        # memorys = response.json()   # a-mem
        # context = memorys["context"]   # a-mem
        
        
        tokens = context.split()[:8192]
        truncated_context = ' '.join(tokens)
        mem_prompt = f"\nHere are some memorys for your reference:\n{truncated_context}\n"
